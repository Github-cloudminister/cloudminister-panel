...
def PanelEnabledSurveyFetchAPI():
    ...
    try:
        ...
        # response = requests.get(f'{settings.OPINIONSDEALSNEW_BASE_URL}survey-create-opinionsdeal', json={'survey':survey_list})
        def survey_preescreener_data(survey_prescreener):

            if len(survey_prescreener) != 0:
                response = requests.get(f'{settings.OPINIONSDEALSNEW_BASE_URL}survey-create-opinionsdeal', json={'survey':survey_prescreener})
            else:
                return

        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
            yield_results = list(executor.map(survey_preescreener_data, survey_preescreener_list))

        survey_number_list_obj = project_grp_list.values_list('project_group_number', flat=True)

        panel_survey_status_update_response = requests.post(f'{settings.OPINIONSDEALSNEW_BASE_URL}survey-status-update-opinionsdeal', json=list(survey_number_list_obj))
        
        survey_priority_resp = RountingPriority.objects.filter(project_group__project_group_status = "Live",project_group__enable_panel = True).values_list(
            'project_group__project_group_number', flat=True
        )

        panel_priorrity_response = requests.post(f'{settings.OPINIONSDEALSNEW_BASE_URL}survey-priority-create-opinionsdeal', json=list(survey_priority_resp))


        # if response.status_code == 200:
        CeleryRunLog.objects.create(taskname = 'Opinions Deal Survey Updated')
        return_mesage = "Opinions Deal Survey Updated"
        # else:
        #     return_mesage = f"Opinions Deal api error {response.status_code}"
        return JsonResponse({"message": return_mesage})
    except Exception as e:
        OpinionsDealErrorLog.objects.create(client_name = "Opinions Deal", line_number = "41", error_details = e)
        return_mesage = "Opinions Deal Error Found"
        return JsonResponse({"message": return_mesage})
